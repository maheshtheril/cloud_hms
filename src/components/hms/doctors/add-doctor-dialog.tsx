...
export function AddDoctorDialog({ isOpen, onClose, departments: initialDepartments, roles, specializations }: AddDocFormProps) {
    ...
    const [activeTab, setActiveTab] = useState<'identity' | 'professional' | 'documents'>('identity')

    const tabs = [
        { id: 'identity', label: 'Identity & Access', icon: UserCheck },
        { id: 'professional', label: 'Professional Details', icon: Briefcase },
        { id: 'documents', label: 'Documents & Bio', icon: FileText },
    ] as const
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
            <div className="relative w-full max-w-5xl bg-slate-50 dark:bg-slate-900 rounded-xl shadow-2xl max-h-[90vh] flex flex-col overflow-hidden border border-slate-200 dark:border-slate-800 transition-all">
                ...
                {/* Tab Navigation */}
                <div className="px-6 pt-4 bg-white dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 shrink-0">
                        ...
                        {tabs.map(tab => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`pb-3 flex items-center gap-2 text-sm font-bold border-b-2 transition-colors whitespace-nowrap ${isActive
                ...
                <form onSubmit={async (e) => {
                    e.preventDefault();
                    // ... (submission logic remains same, reusing existing validation logic below)
                    ...
                    <div className="p-6">
                        ...
                        {/* TAB 1: IDENTITY */}
                        <div className={activeTab === 'identity' ? 'grid grid-cols-12 gap-6 animate-in slide-in-from-right-4 fade-in duration-300' : 'hidden'}>
                                ...
                                <div className="bg-white dark:bg-slate-950 p-5 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
                                    ...
                                    <div className="grid grid-cols-12 gap-6">
                                        <div className="col-span-6">
                                            ...
                                            <input type="text" name="first_name" required className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-md text-sm focus:ring-2 focus:ring-blue-500 outline-none transition-all" />
                                        </div>
                                        <div className="col-span-6">
                                            ...
                                            <input type="text" name="last_name" required className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-md text-sm focus:ring-2 focus:ring-blue-500 outline-none transition-all" />
                                        </div>
                                        <div className="col-span-12">
                                            ...
                                            <div className="relative">
                                                ...
                                                <input type="email" name="email" required className="w-full pl-9 px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-md text-sm focus:ring-2 focus:ring-blue-500 outline-none transition-all" />
                                            </div>
                                        </div>
                                        <div className="col-span-12">
                                            ...
                                            <div className="relative">
                                                ...
                                                <input type="text" name="license_no" className="w-full pl-9 px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-md text-sm focus:ring-2 focus:ring-blue-500 outline-none transition-all" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        {/* TAB 2: PROFESSIONAL */}
                        <div className={activeTab === 'professional' ? 'grid grid-cols-12 gap-6 animate-in slide-in-from-right-4 fade-in duration-300' : 'hidden'}>
                            <div className="col-span-12 space-y-6">
                                <div className="bg-white dark:bg-slate-950 p-5 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
                                    <h3 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider mb-4 border-b border-slate-100 dark:border-slate-800 pb-2 flex items-center gap-2">
                                        <Briefcase className="h-4 w-4 text-emerald-500" /> Organizational Role
                                    </h3>
                                    <div className="grid grid-cols-12 gap-6">
                                        <div className="col-span-6">
                                ...
                                <div className="bg-white dark:bg-slate-950 p-5 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
                                    <h3 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider mb-4 border-b border-slate-100 dark:border-slate-800 pb-2 flex items-center gap-2">
                                        <Clock className="h-4 w-4 text-indigo-500" /> Shifts & Availability
                                    </h3>
                                    <div className="grid grid-cols-12 gap-6">
                                        <div className="col-span-6">
                        ...
                        {/* TAB 3: DOCUMENTS */}
                        <div className={activeTab === 'documents' ? 'grid grid-cols-12 gap-6 animate-in slide-in-from-right-4 fade-in duration-300' : 'hidden'}>
                            <div className="col-span-12 space-y-6">
                                <div className="bg-white dark:bg-slate-950 p-5 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
                                    <h3 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider mb-4 border-b border-slate-100 dark:border-slate-800 pb-2 flex items-center gap-2">
                                        <Camera className="h-4 w-4 text-purple-500" /> Biometrics & Visuals
                                    </h3>
                                    <div className="grid grid-cols-12 gap-6">
                                        <div className="col-span-6">
                                            <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-2">Profile Photo</label>
                                            <FileUpload onUploadComplete={(url) => setProfileImageUrl(url)} folder="staff/profiles" label="Click to Upload" accept="image/*" showCamera={true} />
                                            <input type="hidden" name="profile_image_url" value={profileImageUrl} />
                                        </div>
                                        <div className="col-span-6">
                                            <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-2">Signature</label>
                                            <FileUpload onUploadComplete={(url) => setSignatureUrl(url)} folder="staff/signatures" label="Upload Signature" accept="image/*" />
                                ...
                                <div className="bg-white dark:bg-slate-950 p-5 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
                                    <h3 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider mb-4 border-b border-slate-100 dark:border-slate-800 pb-2 flex items-center gap-2">
                                        <FileText className="h-4 w-4 text-orange-500" /> Compliance Documents
                                    </h3>
                                    <div className="space-y-4">
                                        {[{ id: 'license', label: 'License Copy' }, { id: 'id_proof', label: 'ID Proof (Govt)' }].map(doc => (
                                            <div key={doc.id}>
                                                ...
                                                <FileUpload onUploadComplete={(url) => setDocumentUrls(prev => ({ ...prev, [doc.id]: url }))} folder="staff/documents" label="Upload PDF/IMG" accept="application/pdf,image/*" />
                                            </div>
                                        ))}
                ...
                {/* Footer Controls */}
                <div className="p-4 bg-slate-50 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 shrink-0 flex justify-end gap-3 rounded-b-xl">
                    <button type="button" onClick={onClose} className="px-5 py-2.5 rounded-lg text-sm font-medium text-slate-600 hover:bg-slate-200 transition-colors">
                        Cancel
                    </button>
                    <button
                        onClick={(e) => { const form = e.currentTarget.closest('.relative')?.querySelector('form'); if (form) form.requestSubmit(); }}
