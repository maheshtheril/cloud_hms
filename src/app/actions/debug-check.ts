...
export async function runDiagnostics(email: string) {
    ...
    return {
        ...
        analysis: {
            ...
            crmExistsInMaster: masterModules.some(m => m.key === 'crm')
        }
    }
}
...
export async function enableCRM(email: string) {
    ...
    const crmMaster = await prisma.modules.findUnique({ where: { key: 'crm' } });
    if (!crmMaster) {
        // Create it on the fly if missing
        await prisma.modules.create({
            data: { key: 'crm', name: 'CRM', description: 'Customer Relationship Management' }
        }).catch(() => { })
    }
