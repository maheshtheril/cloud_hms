...
/**
 * [RCM] Generate Consolidated OP Booking Invoice
 * Combines Registration and Consultation dues into a single draft invoice.
 */
export async function generateOPBookingInvoice({
    patientId,
    clinicianId,
    ...
    try {
        const clinician = await prisma.hms_clinicians.findUnique({
            ...
            select: { consultation_fee: true, first_name: true, last_name: true }
        });
        ...
        if (includeRegistration) {
            ...
            const billingSettings = (settings?.value as any) || {};
            registrationFee = Number(billingSettings.registrationFee) || 150;
            const regProductId = billingSettings.registrationProductId;

            if (regProductId) {
                regProduct = await prisma.hms_product.findUnique({ where: { id: regProductId } });
            }
            if (!regProduct) {
                regProduct = await prisma.hms_product.findFirst({
                    where: {
                        tenant_id: tenantId,
                        name: { contains: 'Registration Fee', mode: 'insensitive' }
                    }
        ...
        if (subtotal <= 0) return { error: "No fees due for this booking" };

        const invNoRes = await getNextVoucherNumber();
        const invoiceNumber = invNoRes.success ? invNoRes.data : `INV-OP-${Date.now().toString().slice(-6)}`;
        ...
        if (consultationFee > 0) {
            lines.push({
                ...
                company_id: companyId,
                line_idx: 1,
                description: `OP Consultation: Dr. ${clinician.first_name} ${clinician.last_name}`,
                quantity: 1,
                unit_price: consultationFee,
                net_amount: consultationFee
            });
        ...
        if (registrationFee > 0) {
            lines.push({
                ...
                description: "Patient Registration & Identity Service",
                quantity: 1,
                unit_price: registrationFee,
                net_amount: registrationFee
            });
        }
        ...
        const invoice = await prisma.hms_invoice.create({
            data: {
                ...
                status: 'draft',
                billing_metadata: {
                    source: 'op-clinical-triage-consolidated',
                    description: 'Unified booking and registration fees'
                },
                branch_id: session.user.current_branch_id,
                created_by: session.user.id,
                hms_invoice_lines: {
        ...
        console.error("FAILED_TO_GENERATE_OP_BOOKING_INVOICE:", err);
        return { error: err.message || "RCM_FAILURE: Could not generate booking record." };
    }
}
