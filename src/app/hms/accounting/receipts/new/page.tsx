...
import { ArrowLeft, Save, Loader2, Calendar, CreditCard, User } from 'lucide-react';
...
export default function NewReceiptPage() {
    ...
    const [status, setStatus] = useState('draft'); // For UI visual only, backend handles posting
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!partnerId) {
            toast({ title: "Validation Error", description: "Please select a Payer/Patient", variant: "destructive" });
            return;
        }
            ...
            <div className="border-b border-white/5 bg-neutral-900/50 backdrop-blur-xl sticky top-0 z-40">
                <div className="max-w-[1000px] mx-auto px-6 h-20 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-xl font-semibold text-white tracking-tight">New Receipt</h1>
                            <p className="text-xs text-neutral-500 mt-1">Record money received</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-medium text-neutral-500 uppercase tracking-wider flex items-center gap-2">
                                    <User className="h-3 w-3" /> Received From
                                </label>
                                <SearchableSelect
                                    value={partnerId}
                                    ...
                                    isDark
                                />
                                <p className="text-[10px] text-neutral-600">Search by Name or Patient ID</p>
                            </div>

                            {/* Amount - HERO */}
                            <div className="space-y-2">
                                <label className="text-xs font-medium text-emerald-500 uppercase tracking-wider flex items-center gap-2">
                                    Amount Received
                                </label>
                                <div className="relative group">
                                    ...
                                    <input
                                        ...
                                        placeholder="0.00"
                                        className="w-full pl-10 pr-4 py-4 bg-neutral-950 border border-emerald-500/30 rounded-xl text-3xl font-bold text-white placeholder:text-neutral-800 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/50 transition-all font-mono"
                                        autoFocus
                                    />
                                </div>
                            </div>
                    ...
                    <div className="flex justify-end gap-4">
                        ...
                        <button
                            ...
                            className="px-8 py-3 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-medium shadow-lg shadow-emerald-900/20 hover:shadow-emerald-900/40 transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            {isSubmitting ? (
                                <>
