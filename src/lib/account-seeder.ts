...
export async function ensureDefaultAccounts(companyId: string, tenantId: string) {
    ...
    // 2. Seed Standard COA with Localized Names (World Standard Template)
    const accounts = [
        // --- ASSETS (1000-1999) ---
        ...
        { code: '2210', name: `${taxLabel} Input (Paid on Purchases)`, type: 'Liability' }, // Input Tax
    ...
    for (const acc of accounts) {
        await prisma.accounts.upsert({
            ...
            update: {
                // Update Name (e.g. Tax label upgrade)
                name: acc.name,
                type: acc.type
            },
            create: {
